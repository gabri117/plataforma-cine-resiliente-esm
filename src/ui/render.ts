        ...
        card.innerHTML = `
            <div class="relative overflow-hidden aspect-[2/3] cursor-pointer" onclick="openDetailModalById('${movie.id}')">
                ...
                <div class="absolute top-3 left-3 bg-slate-950/80 backdrop-blur-md px-2.5 py-1 rounded-lg text-xs font-bold text-brand-gold border border-slate-800 flex items-center gap-1">
                    <i class="fa-solid fa-star"></i> ${movie.rating.average}
                </div>
            ...
            <div class="p-4 flex flex-col flex-grow justify-between gap-3">
                <div>
                    <span class="text-[11px] font-medium text-brand-red uppercase tracking-wide block mb-1">
                        ${translateGenre(movie.genres[0] || 'Drama')}
                    </span>
...
export function updateHeroBanner(movie) {
    if (!movie) return;
    document.getElementById('heroBg').style.backgroundImage = `url('${movie.image.original}')`;
    document.getElementById('heroTitle').textContent = movie.name;
    document.getElementById('heroOverview').textContent = movie.summary.replace(/<\/?[^>]+(>|$)/g, "");

    document.getElementById('heroMeta').classList.remove('hidden');
    document.getElementById('heroRating').textContent = movie.rating.average;
    document.getElementById('heroGenre').textContent = translateGenre(movie.genres[0]);
    document.getElementById('heroYear').textContent = movie.year;

    document.getElementById('heroBookBtn').onclick = () => openTicketModal(movie);
    document.getElementById('heroDetailBtn').onclick = () => openDetailModal(movie);
}
...
export function showToast(message, iconClass = 'fa-circle-check', iconColor = 'text-emerald-400') {
    ...
    document.getElementById('toastIcon').className = `fa-solid ${iconClass} ${iconColor}`;
    toast.classList.remove('translate-y-20', 'opacity-0');
    setTimeout(() => toast.classList.add('translate-y-20', 'opacity-0'), 3000);
}

export function openDetailModalById(id) {
    const movie = state.movies.find(m => String(m.id) === String(id)) || state.filteredMovies.find(m => String(m.id) === String(id));
...
export function openDetailModal(movie) {
    document.getElementById('modalCover').src = movie.image.original;
    document.getElementById('modalTitle').textContent = movie.name;
    document.getElementById('modalGenre').textContent = movie.genres.map(translateGenre).join(' \u2022 ');
    document.getElementById('modalRating').textContent = movie.rating.average;
    document.getElementById('modalRuntime').querySelector('span').textContent = `${movie.runtime} min`;
    document.getElementById('modalSummary').innerHTML = movie.summary;

    const favBtn = document.getElementById('modalFavBtn');
    const isFav = state.favorites.has(movie.id);
    favBtn.querySelector('i').className = `${isFav ? 'fa-solid text-brand-red' : 'fa-regular'} fa-heart`;
    favBtn.onclick = () => {
        toggleFavorite(movie.id);
        favBtn.querySelector('i').className = `${state.favorites.has(movie.id) ? 'fa-solid text-brand-red' : 'fa-regular'} fa-heart`;
    };
...
export function openTicketModal(movie) {
    ...
    document.getElementById('ticketMovieTitle').textContent = movie.name;
    updateTicketTotal();
...
export function updateHeroReviews(count) {
    ...
    document.getElementById('heroReviewCount').textContent = count;
}
...
export function filterAndRenderMovies() {
    state.filteredMovies = state.movies.filter(movie => {
        ...
        return movie.genres.some(g => g.toLowerCase().includes(state.activeFilter.toLowerCase()));
    });
    renderGrid();
}
